...
void JTree::construct( const FactorGraph &fg, const std::vector<VarSet> &cl, bool verify ) {
    ...
    ofstream output;
    if( props.verbose >= 3 ){
        output.open("regiongraph.jt", ios::out);
        output << "// Weightedgraph: " << JuncGraph << endl;
    }
    ...
    if( props.verbose >= 3 ){
        output << "// Spanning tree: " << RTree << endl;
        output.close();
    }
...
void JTree::GenerateJT( const FactorGraph &fg, const std::vector<VarSet> &cl ) {
    ...
    ofstream output;
    if( props.verbose >= 3 ){
        output.open("regiongraph.jt", ios::app);
        output << "// Regiongraph generated by JTree::GenerateJT: \n" << *this << endl;
    }
}
