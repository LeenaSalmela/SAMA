...
class BP : public DAIAlgFG {
    ...
    public:
        ...
        struct Properties {
             ...
             */
            DAI_ENUM(UpdateType,SEQFIX,SEQRND,SEQMAX,PARALL,SEQMAX0L,RESIDSPLASH);
    ...
    //@{
        ...
        /// Copy constructor
        BP( const BP &x ) : DAIAlgFG(x), _edges(x._edges), _edge2lut(x._edge2lut), _lut(x._lut), _vlut(x._vlut), _fnode2lut(x._fnode2lut), _maxdiff(x._maxdiff), _iters(x._iters), _sentMessages(x._sentMessages), _oldBeliefsV(x._oldBeliefsV), _oldBeliefsF(x._oldBeliefsF), _updateSeq(x._updateSeq), props(x.props), recordSentMessages(x.recordSentMessages) {
	...
	void increasePriority(size_t i, size_t _I, Real r);
        ...
        void findMaxNodeResidual(size_t &I);
