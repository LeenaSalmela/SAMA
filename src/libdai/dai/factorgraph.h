...
class FactorGraph {
    ...
    //@{
        ...
        /// Get a splash of Factor nodes (Gonzalez et al. 2009)
        /**
         * The returned vector contains the BFS order of Factor nodes from a search starting at node V with a depth 
         * of 2h (we skip variable nodes as we only consider messages I->i)
         */
        std::vector<size_t> getSplash(size_t V, size_t h) const;
        
        typedef std::multimap<Real,size_t> vLutType;
        std::vector<size_t> getResidualSplash(size_t V, size_t h, const std::vector<vLutType::iterator>& fnode2lut, Real tol) const;
    //@}
    ...
    //@{
        ...
        virtual void fg2Cytoscape (std::ostream &osa, std::ostream &osn ) const;
        virtual void dBGfg2Cytoscape( std::ostream &osa, std::ostream &osn, std::map<size_t,size_t>& nodeV) const;
