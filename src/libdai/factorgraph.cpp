...
void FactorGraph::fg2Cytoscape( std::ostream &osa, std::ostream &osn ) const {
        osa << "source" << "\t" << "target" << std::endl;
        osn << "ID" << "\t" << "node type" << std::endl;
        for ( size_t I = 0; I < nrFactors(); I++ ) {
                osn << "f" << I << "\t" << 1 << std::endl;
                for( VarSet::const_iterator i = factor(I).vars().begin(); i != factor(I).vars().end(); i++ )
                        osa << "f" << I << "\t" << "v" << i->label() << std::endl;
        }
        for( size_t i = 0; i < nrVars(); i++ )
                osn << "v" << var(i).label() << "\t" << 0 << std::endl;
}

void FactorGraph::dBGfg2Cytoscape( std::ostream &osa, std::ostream &osn, std::map<size_t,size_t>& nodeV) const {
        osa << "source" << "\t" << "target" << std::endl;
        osn << "ID" << "\t" << "node type" <<"\t" << "dBG ID" << std::endl;
        for ( size_t I = 0; I < nrFactors(); I++ ) {
                osn << "f" << I << "\t" << 2 << std::endl;
                for( VarSet::const_iterator i = factor(I).vars().begin(); i != factor(I).vars().end(); i++ )
                        osa << "f" << I << "\t" << "v" << i->label() << std::endl;
        ...
        for( size_t i = 0; i < nrVars(); i++ ){
                size_t pgm_id = var(i).label();
                bool dBGnode = (nodeV.find(pgm_id) != nodeV.end());
                if(dBGnode)
                        osn << "v" << pgm_id << "\t" << 0 << "\t" << nodeV[pgm_id] << std::endl;
                else
                        osn << "v" << pgm_id << "\t" << 1 << std::endl;
        }
}
...
std::vector< size_t > dai::FactorGraph::getSplash(size_t V, size_t h) const
{
        vector<size_t> bfs_order;
        vector<bool> visited(nrFactors(),false); // if h is small maybe use a set?
        std::queue<pair<size_t, size_t>> Q;
        Q.push(make_pair(V,0));
        visited[V] = true;
        while(! Q.empty()){
                pair<size_t,size_t> q_el = Q.front();
                Q.pop();
                bfs_order.push_back(q_el.first);
                
                if(q_el.second >= h)
                        continue;
                
                bforeach(const Neighbor& i, nbF(q_el.first)){
                    bforeach(const Neighbor& I, nbV(i)){
                        if( ! visited[I]){
                            Q.push(make_pair(I,q_el.second+1));
                            visited[I] = true;
                        }
                    }    
                }
...
std::vector<size_t> FactorGraph::getResidualSplash(size_t V, size_t h, const std::vector<vLutType::iterator>& fnode2lut, Real tol) const
{
        vector<size_t> bfs_order;
        vector<bool> visited(nrFactors(),false); // if h is small maybe use a set?
        std::queue<pair<size_t, size_t>> Q;
        ...
        while(! Q.empty()){
                ...
                bforeach(const Neighbor& i, nbF(q_el.first)){
                        bforeach(const Neighbor& I, nbV(i)){
                                if( (! visited[I]) && (fnode2lut[q_el.first]->first > tol)){
                                        Q.push(make_pair(I,q_el.second+1));
                                        visited[I] = true;
                                }
                        }    
